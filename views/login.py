...
def iniciar_login():  # 👈 Esta función es llamada desde main.py
    ...
    def crear_usuario_ui():
        ...
        tk.Label(ventana, text="Nombre completo:", bg="#B3E5FC").pack(pady=5)
        entry_nombre = tk.Entry(ventana)
        entry_nombre.pack()

        tk.Label(ventana, text="Fecha de nacimiento (DD/MM/AAAA):", bg="#B3E5FC").pack(pady=5)
        entry_fecha = tk.Entry(ventana)
        entry_fecha.pack()

        tk.Label(ventana, text="DNI:", bg="#B3E5FC").pack(pady=5)
        entry_dni_nuevo = tk.Entry(ventana)
        entry_dni_nuevo.pack()

        tk.Label(ventana, text="Email:", bg="#B3E5FC").pack(pady=5)
        entry_email = tk.Entry(ventana)
        entry_email.pack()

        tk.Label(ventana, text="Teléfono:", bg="#B3E5FC").pack(pady=5)
        ...
        tk.Button(
            ventana, text="Guardar usuario", bg="#B3E5FC", fg="grey",
    ...
    ruta_logo = os.path.join(os.path.dirname(__file__), "imagen", "logo3.png")
    try:
        logo_img = Image.open(ruta_logo).convert("RGBA")
        logo_img = logo_img.resize((380, 280), Image.LANCZOS)
        logo_tk = ImageTk.PhotoImage(logo_img)
        lbl_logo = tk.Label(frame, image=logo_tk, bg="#B3E5FC")
        lbl_logo.image = logo_tk
        lbl_logo.pack(pady=(0, 0))
    except:
        tk.Label(frame, text="🐾 SystemPet 🐾", bg="#B3E5FC",
                 font=("Aptos Black", 18, "bold")).pack(pady=10)

    tk.Label(frame, text="Ingrese su DNI:", bg="#B3E5FC",
             font=("Aptos", 14, "bold")).pack(pady=(2, 0))
    entry_dni = tk.Entry(frame, font=("Arial", 14), width=20)
    entry_dni.pack(pady=5)

    tk.Button(frame, text="Ingresar", bg="#C8F0F5", fg="grey",
              font=("Aptos Bold", 11, "bold"), relief="raised",
              command=verificar_login).pack(pady=5)

    tk.Button(frame, text="Crear usuario", bg="#C8F0F5", fg="grey",
              font=("Aptos", 11, "bold"), relief="raised",
